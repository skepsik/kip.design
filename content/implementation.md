...
- своп: отключён/зашифрован на уровне машины (дисциплина окружения; kip не
  проверяет) либо признаём риск строчкой;
- редакторы оставляют бэкапы/undo — `close` подметает корень vault (не заходя в
  `.kip`), белый список мусора редакторов;
- VK — только в RAM процесса CLI на время команды (сессия packer в этом
  процессе), не в OS-keychain. Долгоживущий агент с TTL — после MVP (§2.6).
...
- Ключевая иерархия (дизайн 5.2а): случайный vault key; объекты шифруются VK. Фраза
  ...
  **argon2id**
  (memory-hard; параметры — константы конфигурации, целевой бюджет ~1s unlock на
  эталонной машине — не гарантия wall-time всего CLI), не итерированный SHA-S2K;
  при заведении фразы `init` предупреждает
...
- **Объект слота (раскладка на диске):** тот же wire, что у прочих блобов;
  ...
  - ключ AES-256: **argon2id** от фразы, соль = `salt` из обёртки, выход 32 байта;
    прод-параметры (константы конфигурации, целевой бюджет ~1s unlock на
    эталонной машине): memory 65536 KiB, time cost 3, parallelism 4;
  - имя объекта слота: `SHA-256( vault-id_ascii || 0x00 || passphrase_utf8 )` →
    lowercase hex первых 16 байт дайджеста (32 символа); соль имени — открытый
    **vault-id** (16 случайных байт → lowercase hex, объект россыпи с именем
    `vault-id`).
- `unlock`: вычислить имя слота → прочитать → развернуть VK → держать в RAM
  процесса до конца вызова CLI (пока жива сессия packer). Успешное развёртывание VK
  **и есть** проверка слота — отдельной подсистемы самопроверки не существует; сверка
  и починка копий слота (локальная ↔ remote) — обязанность `fsck`, не unlock.
- `passwd` — цепочка с фиксированным порядком: записать новый слот → раундтрип-
  верификация (прочитать назад, развернуть VK новой фразой, сверить с VK уже в
  процессе) → удалить старый → **purge старого из истории** (обязательная часть
  семантики, дизайн 5.2а; в тексте дизайна иногда «`passwd --purge`» — тот же
  обязательный purge, не опциональный режим). На уровне дельты это `D` + `A` (имя
  слота следует за фразой), два синглетона нормальной формы; обрыв цепочки в любой
  точке оставляет рабочий слот — покрывается тестом цепочки.
...
- Каждый запуск CLI с packer: отдельный unlock (фраза — env/file/tty). Долгоживущий
  агент (фраза/VK с TTL между процессами) — после MVP (§2.6).
...
**Синхронизация эпох между устройствами** — контракт будущего агента при живой
сессии (в MVP агента нет; каждый CLI — свой unlock; спящий на одном устройстве, как
merge-валидность):
...
- Локальная модель угроз: жизненный цикл таблицы = жизненному циклу дерева
  ...
  `~/.kip/<id>` — Rejected §1.1); своп — дисциплина машины (не проверка в kip); VK —
  в процессе CLI, не в OS-keychain; долгоживущий агент — после MVP (§2.6); FDE —
  фон (дизайн 5.2в).
...
- Упаковщик: VK + слоты; соль имени слота — vault-id (случайный, при `init`,
  ...
  свободны; сигнатура формата принята); слот под argon2id (целевой бюджет параметров
  ~1s unlock на эталонной машине, не гарантия wall-time CLI; `init` предупреждает
  о словарных фразах — короче 12 символов + top-N словарь, без библиотек); `passwd` =
  «новый → раундтрип-верификация (сверка с VK в процессе) → удаление → purge»
  (`D`+`A`), обрыв в любой точке оставляет рабочий слот — покрывается тестом цепочки;
  учётная данная одна — фраза; wire-формат блоба — заголовок (`pack_version`,
  `vk_epoch`) + ciphertext, с MVP `vk_epoch=1`; `rotate-vk` и синхронизация эпох
  (агент) — после MVP (§2.6); ротация VK — аварийная (перешифрование + отсечение
  истории + `vk_epoch++`); unlock сам есть проверка слота, сверка копий — в `fsck`;
  `kip rm` / `--cached` обратимы историей; `kip rm --purge` — явный выжиг для
  нератируемых секретов; scan — crate `ignore` + `.kipignore` / `.kip/ignore`;
  бюджеты сложности мелочей — 2.5; холодный старт начинается со слота.
